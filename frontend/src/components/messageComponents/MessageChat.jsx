...
useEffect(() => { //scroll effect
  ...
  return (
     <div ref={scrollRef} className={`flex ${isUserSend ?'justify-end':'justify-start'} w-full m-3 pr-2`}>
      <div className={` ${isUserSend ?'bg-violet-950 rounded-br-none ':'bg-gray-900/50 rounded-bl-none'} max-w-[80%] md:max-w-[65%] min-w-16 w-fit p-2 px-3 pb-1 text-sm text-white rounded-3xl text-start break-words`}>
        ...
        <div className='flex justify-end items-center inset-0   cursor-default'>
          <p className='text-[0.6rem] font-semibold text-white/70'>{timeString}</p>
          {isUserSend && (messageData?.isRead=='send'?<BiCheck className='fill-slate-100 font-semibold text-sm -mr-1'/>:<BiCheckDouble className={`${messageData?.isRead=='seen'?'fill-blue-500':'fill-gray-100'} font-semibold text-base -mr-1`}/>)}
        </div>
        </div>
    </div>
  )
