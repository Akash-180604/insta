...
import { setFollowingsData } from '../../redux/userSlice';
...
const GetFollowings = () => {
    ...
    const {userData, followingsData} = useSelector(state=>state.user);

    useEffect(()=>{
        const getFollowingsFnc = async ()=>{
        const result = await axios.get(`${import.meta.env.VITE_SERVER_URL}/user/getFollowings/${userName}`,{withCredentials:true});
            dispatch(setFollowingsData(result.data));
    ...
    if (val.trim()) {
        const filtered = followingsData.filter(user=>{
        return(
        ...
        setFilteredList(followingsData);
    }
  ...
  return (
        <div className='w-screen min-h-screen bg-black flex justify-center overflow-auto '>
            <div className='w-full max-w-[500px] bg-gray-950 rounded-2xl pb-16'>
                <div className='w-full max-w-[500px] p-3 flex items-center gap-6 px-7 fixed bg-gray-950 '>
                    ...
                    <h1 className='font-bold text-xl text-gray-50 cursor-default'>{ userName }  <span className='font-semibold'>Followings</span></h1>
                </div>
                <div className='w-full py-2 px-8 pt-16 '>
                    <input type="text" placeholder='search followings....' value={input} onChange={searchFnc} className='outline-none border-none w-full px-3 py-1 text-white bg-gray-700 text-sm rounded-[24px]'/>
                </div>
                <div className='w-full py-2 px-20 pt-2 flex items-center gap-8'>
                    <div onClick={()=>navigate(`/profile/followers/${userName}`)} className='hover:border-gray-400 hover:text-gray-400 flex-1 text-gray-500 text-sm font-semibold text-center py-2 border-b-2 border-gray-500 cursor-pointer'> Followers </div>
                    <div className='flex-1 text-white text-sm font-semibold text-center py-2 border-b-2 border-white cursor-pointer'>{followingsData?.length} Followings</div>
      ...
      <div className='h-full w-full bg-white rounded-full'>
        <img src={user?.profileImage || dp} alt="" className='h-full w-full rounded-full object-cover'/>
            ...
            <div onClick={(e)=>{followFnc(user?._id,userData._id,dispatch);  e.stopPropagation()}} className=' text-gray-900 text-xs font-semibold bg-white px-2.5 py-1.5 rounded-full cursor-pointer'>
            {((userData?.followings).includes(user?._id))?'Following' : 'Follow'}
            {/* {((user?.followings)?.includes(userData._id))?'Following' : 'Follow'} */}
            ...
            {filteredList?.length === 0 && (
              <p className="text-gray-500 text-center text-sm mt-8">No Folloewings user found</p>
            )}
